...
func TestCommandDisptacher_Dispatch(t *testing.T) {
	...
	dispatcher.AddSubscriber(mockCommand, func(ctx context.Context, command *weos.Command, eventRepository weos.EventRepository, projection weos.Projection) error {
		handlersCalled += 1
		return nil
	})
	...
	dispatcher.AddSubscriber(&weos.Command{
		...
		Payload:  nil,
		Metadata: weos.CommandMetadata{},
	}, func(ctx context.Context, event *weos.Command, eventRepository weos.EventRepository, projection weos.Projection) error {
		handlersCalled += 1
