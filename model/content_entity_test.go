...
func TestContentEntity_AutoGeneratedID(t *testing.T) {
	...
	t.Run("Generating id type is string and the format is not specified  ", func(t *testing.T) {
		...
		err = json.Unmarshal(payload3, &p3)
		if err != nil {
			t.Errorf("unexpected error unmarshalling entity; %s", err)
		}
		if p3["id"] == nil {
			t.Fatalf("expected id for third entity to be generated got nil")
		}
	})
	t.Run("Generating id type is integer and the format is not specified ", func(t *testing.T) {
		contentType4 := "Pages"
		entityFactory4 := new(model.DefaultEntityFactory).FromSchemaAndBuilder(contentType4, api.Swagger.Components.Schemas[contentType4].Value, schemas[contentType4])
		if err != nil {
			t.Fatalf("error setting up entity factory")
		}
		p4 := map[string]interface{}{"title": "old blogs"}
		payload4, err := json.Marshal(p4)
		if err != nil {
			t.Errorf("unexpected error marshalling entity; %s", err)
		}
		payload4, err = model.GenerateID(payload4, entityFactory4)
		if err != nil {
			t.Errorf("unexpected error generating id; %s", err)
		}
		err = json.Unmarshal(payload4, &p4)
		if err != nil {
			t.Errorf("unexpected error unmarshalling entity; %s", err)
		}
		if p4["id"] == nil {
