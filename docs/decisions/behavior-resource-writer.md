...
- **Conservative ceiling.** The depth counter only tracks cascades within a *single* request-scoped `ResourceService` call. The deepest legitimate in-request cascade in the education preset is course-instance → education-event → attendance-record (3 levels, fired when a course-instance is created or updated). Payment-driven invoice settlement and attendance-driven invoice generation start their own request trees (each at depth 1) so they do not stack on top of schedule generation. The limit of 8 leaves generous headroom for future presets whose hooks chain further while still failing fast on cycles.
