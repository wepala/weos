...
through the proxy, or a trusted issuer that has authenticated the person may post the
assertion itself and relay the instance's answer — the status, the body and every
`Set-Cookie` — on the host that serves the instance. Relaying the answer is allowed only
where the browser reaches the instance on the same origin the answer is relayed on, which
is what an issuer that fronts the instance gives (the door serves the instance on the
door's own origin). Where that holds, the instance's `Set-Cookie` lands in the browser
...
- Bad: an instance started on demand must be healthy before the assertion is minted;
  that is the issuer's obligation (mint after `/api/health`, and mint a fresh assertion — a
