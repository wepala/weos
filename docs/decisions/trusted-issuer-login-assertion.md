...
through the proxy, or a trusted issuer that holds the person's credential may post the
assertion itself and relay the instance's answer — the status, the body and every
`Set-Cookie` — on the host that serves the instance. Either way the instance's
`Set-Cookie` lands in the browser exactly as it does for `/auth/password-login`: the
cookie must reach the browser, and a relayed one does, because it arrives on the same host
the browser is already talking to. An issuer that posts the assertion itself keeps two
duties: it mints a fresh assertion, with a new `jti`, for every attempt, and it mints one
only after the instance answers that it is ready (see "Consequences").
