...
func TestSecurityPathInitializer(t *testing.T) {
	api, err := rest.New("./fixtures/blog-security-path.yaml")
	if err != nil {
		t.Fatalf("unexpected error loading api '%s'", err)
	}
	schemas := rest.CreateSchema(context.TODO(), api.EchoInstance(), api.Swagger)
	...
	t.Run("security set on a path", func(t *testing.T) {

		ctxt, err := rest.SecurityPathInitializer(baseCtxt, api, "/blogs", http.MethodPost, api.Swagger, api.Swagger.Paths["/blogs"], api.Swagger.Paths["/blogs"].Post)
		if err != nil {
			t.Fatalf("unexpected error loading api '%s'", err)
		}

		middleware := rest.GetOperationMiddlewares(ctxt)
		if middleware == nil {
			t.Fatalf("expected middleware to be in the context")
		}
